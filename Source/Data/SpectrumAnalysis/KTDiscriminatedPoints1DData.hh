    ...
    class KTDiscriminatedPoints1DData : public Nymph::KTExtensibleData< KTDiscriminatedPoints1DData >
    {
        public:
            struct Point
            {
                ...
                Point(double abscissa, double ordinate, double threshold, double mean, double tau, double variance, double neighborhoodAmplitude) : fAbscissa(abscissa), fOrdinate(ordinate), fThreshold(threshold), fMean(mean), fTau(tau), fVariance(variance), fNeighborhoodAmplitude(neighborhoodAmplitude) {}
            };
